...
<style>
  ...
  /* ======= FILTERS ONE LINE ON LARGE SCREENS ======= */
  .filters-row{
      display:flex;
...
<script>
  // ✅ callback appelé automatiquement par Google Maps (via URL callback=initTrajetsMap)
  window.initTrajetsMap = function () {
    try {
      if (typeof window.__startMap === 'function') {
        window.__startMap();
      } else {
        console.error('[Trajets] __startMap not defined');
      }
    } catch (e) {
      console.error('[Trajets] initTrajetsMap crash:', e);
    }
  };
</script>

{{-- ✅ IMPORTANT: on charge Google Maps UNE SEULE FOIS ici --}}
<script
  src="https://maps.googleapis.com/maps/api/js?key={{ config('services.google_maps.key') }}&callback=initTrajetsMap&libraries=places,geometry"
  async defer>
</script>
@endpush
...
<div class="page-shell">
  <div class="page-pad space-y-4">
      ...
      {{-- FILTRES (✅ 1 seule ligne sur grand écran) --}}
      <div class="ui-card p-4 border border-border-subtle rounded-2xl">
          <form id="filtersForm" method="GET" action="{{ url()->current() }}" class="filters-row">
              <input type="hidden" name="page" value="">

              <div class="w-full lg:w-[260px]">
                  <label class="text-xs font-medium text-secondary">Véhicule</label>
              ...
              <div class="w-full lg:w-[210px]">
                  <label class="text-xs font-medium text-secondary">Période</label>
                  <select id="filter-type" name="quick" class="ui-input-style w-full">
                      ...
                      <option value="date"      {{ request('quick')=='date'?'selected':'' }}>Date</option>
                      <option value="range"     {{ request('quick')=='range'?'selected':'' }}>Plage</option>
                  </select>
              </div>

              <div id="single-date" class="hidden w-full lg:w-[190px]">
                  <label class="text-xs font-medium text-secondary">Date</label>
                  <input id="dateInput" type="date" name="date" class="ui-input-style w-full"
                        value="{{ request('date') }}">
              </div>

              <div id="date-range" class="hidden w-full lg:w-[380px]">
                  <label class="text-xs font-medium text-secondary">Plage</label>
                  <div class="flex gap-3">
                      <input id="startDateInput" type="date" name="start_date" class="ui-input-style w-full"
                            value="{{ request('start_date') }}">
                      <input id="endDateInput" type="date" name="end_date" class="ui-input-style w-full"
                            value="{{ request('end_date') }}">
                  </div>
              </div>

              <div class="w-full lg:w-[320px]">
                  <label class="text-xs font-medium text-secondary">Heures</label>
                  <div class="flex gap-3">
                      <input id="startTimeInput" type="time" name="start_time" class="ui-input-style w-full"
                            value="{{ request('start_time') }}">
                      <input id="endTimeInput" type="time" name="end_time" class="ui-input-style w-full"
                            value="{{ request('end_time') }}">
                  </div>
              </div>

              <div class="w-full lg:w-auto lg:ml-auto text-[11px] text-secondary">
                  <i class="fas fa-bolt mr-1 text-primary"></i> Les filtres s’appliquent automatiquement
              </div>
          </form>
      </div>
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function updateUI() {
    ...
    if (type?.value === "date" && single)  single.classList.remove("hidden");
    if (type?.value === "range" && range)  range.classList.remove("hidden");
  }
  ...
  vehicleSearch?.addEventListener('change', () => {
    ...
    const url = urlTemplate.replace('__VID__', encodeURIComponent(String(vid))) + (params.toString() ? ('?' + params.toString()) : '');
...
/**
 * ================================
 *  TRAJETS MAP + MODE + REPLAY
 *  ✅ 1 seul trajet (pas de découpage)
 *  ✅ Mode carte (roadmap/hybrid/satellite/terrain)
 *  ✅ Replay (play/pause/stop/prev/next + vitesse)
 *  ✅ Boot stable (callback Google + DOM ready)
 * ================================
 */
...
window.__startMap = function bootMap() {
  try {
    ...
    // ---- DATA (injectés par Blade) ----
    const tracksRaw = @json($tracks ?? []);
    const focusId = @json($focusId);

    console.log("[Trajets] tracksRaw:", (tracksRaw || []).length, "focusId:", focusId);
    ...
    makeAutoHide(panelMode, () => window.__replayPlaying);
    makeAutoHide(panelReplay, () => window.__replayPlaying);
    ...
    document.addEventListener("click", (e) => {
      ...
      const isBtn =
        (btnMode && (e.target === btnMode || btnMode.contains(e.target))) ||
        (btnReplay && (e.target === btnReplay || btnReplay.contains(e.target)));
    ...
    /* ---------- HELPERS (no split, 1 trajet = 1 polyline) ---------- */
    ...
    function safeTimeMs(t) {
      if (!t) return null;
      const iso = String(t).replace(" ", "T"); // keep local tz
    ...
    function cleanPoints(raw) {
      ...
      for (const p of pts) {
        ...
        if (t1 != null && t2 != null) {
          ...
          if (dt > 0) {
            const v = (d / dt) * 3.6;
            // On ne coupe pas le trajet: on ne "continue" pas.
            // Si tu veux filtrer les points impossibles, décommente:
    ...
    function prepareTrack(tr) {
      let pts = cleanPoints(tr.points || []);
      pts = smooth(pts, 5);
      return { ...tr, __pts: pts, __segments: (pts.length >= 2 ? [pts] : []) };
    }
...
<!-- ✅ Script Google Maps (clé via config) -->
<script
