...
<head>
    ...
    <style>
        ...
        :root {
            --color-primary: #F58220; /* Orange vibrant */
        ...
        /* --- DARK MODE VARIABLES --- */
        .dark-mode {
            --color-bg: #121212; /* Fond très sombre */
            --color-card: #1f2937; /* Cartes et boîtes de dialogue */
            --color-text: #f3f4f6; /* Texte clair */
            --color-input-bg: #374151; /* Fond des champs sombres */
            --color-input-border: #4b5563; /* Bordure des champs sombres */
            --color-secondary-text: #9ca3af; /* Texte secondaire */
            --color-sidebar-bg: #1f2937; /* Sidebar sombre */
            --color-sidebar-text: #f3f4f6;
            --color-sidebar-active-bg: rgba(245, 130, 32, 0.25);
            --color-border-subtle: #374151; /* Bordures sombres */
            --color-navbar-bg: #1f2937; /* Navbar sombre */
            background-color: var(--color-bg);
            color: var(--color-text);
        }
        ...
        .brand .logo-text {
            ...
            font-weight: 800;
            font-size: 1.25rem;
            color: var(--color-primary); /* Le logo (texte) utilise la couleur primaire */
        }
        ...
        .nav-dropdown {
            ...
            padding-left: 2rem;
            background-color: var(--color-sidebar-bg); /* S'assure que le fond reste cohérent */
        }
        .nav-dropdown.open {
            max-height: 500px;
        }
        ...
        .ui-input-style, .ui-textarea-style, .ui-select-style {
        ...
        .ui-input-style:focus, .ui-textarea-style:focus, .ui-select-style:focus {
            outline: none;
        ...
        .ui-table th {
            font-family: var(--font-family);
            background-color: var(--color-border-subtle); /* Utilisation de la couleur de bordure/fond secondaire */
            color: var(--color-text);
            padding: 0.75rem 1rem;
            text-align: left;
        ...
        .btn-primary { background-color: var(--color-primary); color: white; padding: 0.5rem 1.5rem; border-radius: 0.5rem; font-weight: 600; transition: background-color 0.2s, transform 0.1s; font-family: var(--font-family); }
        .btn-primary:hover { background-color: var(--color-primary-dark); transform: translateY(-1px); }
        .btn-secondary { color: var(--color-primary); border: 1px solid var(--color-primary); padding: 0.5rem 1.5rem; border-radius: 0.5rem; font-weight: 600; transition: background-color 0.2s; background-color: transparent; font-family: var(--font-family); }
        .btn-secondary:hover { background-color: rgba(245, 130, 32, 0.1); }
        
        /* Styles divers (conservés) */
        .navbar-icon-btn { width: 40px; height: 40px; display: flex; align-items: center; justify-content: center; border-radius: 50%; font-size: 1.1rem; color: var(--color-text); transition: background-color 0.2s, color 0.2s; cursor: pointer; }
        .navbar-icon-btn:hover { background-color: var(--color-sidebar-active-bg); color: var(--color-primary); }
        ...
        .text-secondary { color: var(--color-secondary-text); }
          ...
          map {
    width: 100%;
    height: 400px; /* Hauteur fixe pour que Leaflet sache où dessiner */
    min-height: 300px;
}
...
#vehicle-form.hidden {
    display: none;
        ...
        @media (min-width: 768px) {
             ...
             .sidebar.collapsed .sidebar-nav a .icon { margin-left: -0.25rem; }
        }

        .toggle-sidebar {
            position: fixed; top: 1rem; left: 1rem; width: 40px; height: 40px;
            align-items: center; justify-content: center; font-size: 1.25rem;
            border-radius: 50%; cursor: pointer; z-index: 25;
            box-shadow: 0 2px 5px rgba(0, 0, 0, 0.1);
            background-color: var(--color-card); color: var(--color-primary);
            border: 1px solid var(--color-primary); transition: background-color 0.2s, color 0.2s;
        }
        .toggle-sidebar:hover { background-color: var(--color-primary); color: var(--color-card); }
...
<body class="light-mode" id="theme-container">
    ...
    <div class="sidebar" id="sidebar">
        ...
        <ul class="sidebar-nav">
            ...
            <li class="nav-item">
                ...
                <ul class="nav-dropdown" id="tracking-menu">
                    <li><a href="{{ route('tracking.users') ?? '#' }}" class="{{ request()->is('tracking_users') ? 'active' : '' }}">Utilisateurs</a></li>
                    <li><a href="{{ route('tracking.vehicles') ?? '#' }}" class="{{ request()->is('tracking.vehicles') ? 'active' : '' }}">Véhicules</a></li>
        ...
        <div class="absolute bottom-0 left-0 w-full p-2 border-t border-solid border-border-subtle" style="background-color: var(--color-sidebar-bg);">
            <a href="{{ route('logout') ?? '#' }}" class="flex items-center p-2 rounded-lg text-secondary hover:text-red-500 transition-colors sidebar-logout-link" title="Déconnexion">
    ...
    <div class="navbar" id="navbar">
        ...
        <div class="flex-grow">
            ...
            <h1 class="text-xl font-semibold hidden sm:block" style="color: var(--color-text);">@yield('title', 'Dashboard')</h1>
        </div>
        ...
        <div class="flex items-center space-x-4">
            ...
            <!-- 1. Toggle Mode Sombre/Clair (TOUJOURS PRÉSENT) -->
             <div class="flex items-center">
                 <span class="text-sm mr-2 pt-0.5 font-orbitron hidden lg:block" style="color: var(--color-secondary-text);" id="mode-label">Mode Clair</span>
            ...
            <div class="relative">
                <button class="navbar-icon-btn" title="Notifications">
                    ...
                    <span class="absolute top-1 right-1 h-2 w-2 bg-red-500 rounded-full border" style="border-color: var(--color-card);"></span>
                </button>
            ...
            <div class="relative" id="user-menu-container">
                <button class="flex items-center space-x-2 p-1 rounded-full hover:bg-sidebar-active-bg transition-colors" id="user-menu-toggle">
                    <img src="https://placehold.co/36x36/F58220/ffffff?text=U" alt="Profile" class="h-9 w-9 rounded-full object-cover border-2 border-primary">
                    <span class="font-semibold hidden lg:block profile-text" style="color: var(--color-text);">John Doe</span>
                </button>
                ...
                <div class="user-dropdown-menu" id="user-menu">
                    ...
                    <a href="{{ route('logout') ?? '#' }}" class="text-red-500"><i class="fas fa-sign-out-alt mr-2"></i> Déconnexion</a>
                </div>
            </div>
...
<script>
    ...
    document.addEventListener('click', (e) => {
        if (userMenu.classList.contains('open') && !userMenu.contains(e.target) && !userMenuToggle.contains(e.target)) {
